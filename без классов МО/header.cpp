...
void Print(char *file_name){
	...
	//-----------------------------------------------------------------
void atree(double **data, int *free, int *depended, int m, int n)
{
...
void Solve(double **data, int *free, int *depended, int m, int n) {
	...
	{
		Ntree *l,*r; //Указатели на новые звенья
		int x; //Ключ звена
	};
	...
	if (!is_solve) cout << "Нет решения\n";
		...
		std::cout << "F  = " << data[m][0] << "\n";
		std::cout << itr << " итерация " << "\n";
		atree(data, free, depended, m, n, tree);
	}
